...
// The original code can be found at `veins/modules/mobility/traci/TraCIScnarioManager.cc` of Veins-5.2.
...
using veins::TraCIScenarioNetManager;
using veins::TraCITrafficLightInterface;

Define_Module(veins::TraCIScenarioNetManager);

const simsignal_t TraCIScenarioNetManager::traciInitializedSignal = registerSignal("org_car2x_veins_modules_mobility_traciInitialized");
const simsignal_t TraCIScenarioNetManager::traciModulePreInitSignal = registerSignal("org_car2x_veins_modules_mobility_traciModulePreInit");
const simsignal_t TraCIScenarioNetManager::traciModuleAddedSignal = registerSignal("org_car2x_veins_modules_mobility_traciModuleAdded");
const simsignal_t TraCIScenarioNetManager::traciModuleRemovedSignal = registerSignal("org_car2x_veins_modules_mobility_traciModuleRemoved");
const simsignal_t TraCIScenarioNetManager::traciTimestepBeginSignal = registerSignal("org_car2x_veins_modules_mobility_traciTimestepBegin");
const simsignal_t TraCIScenarioNetManager::traciTimestepEndSignal = registerSignal("org_car2x_veins_modules_mobility_traciTimestepEnd");

TraCIScenarioNetManager::TraCIScenarioNetManager()
    : connection(nullptr)
    , commandIfc(nullptr)
...
TraCIScenarioNetManager::TypeMapping TraCIScenarioNetManager::parseMappings(std::string parameter, std::string parameterName, bool allowEmpty)
{
...
void TraCIScenarioNetManager::parseModuleTypes()
{
...
void TraCIScenarioNetManager::initialize(int stage)
{
    ...
    EV_DEBUG << "initialized TraCIScenarioNetManager" << endl;
}

bool orderSet = false;

void TraCIScenarioNetManager::init_traci()
{
...
void TraCIScenarioNetManager::preNetworkFinish()
{
...
void TraCIScenarioNetManager::finish()
{
    recordScalar("roiArea", areaSum);
}

void TraCIScenarioNetManager::handleMessage(cMessage* msg)
{
    if (msg->isSelfMessage()) {
        handleSelfMsg(msg);
        return;
    }
    throw cRuntimeError("TraCIScenarioNetManager doesn't handle messages from other modules");
}

void TraCIScenarioNetManager::handleSelfMsg(cMessage* msg)
{
    if (msg == connectAndStartTrigger) {
        connection.reset(TraCIConnection::connect(this, host.c_str(), port));
        commandIfc.reset(new TraCICommandInterfaceScs(this, *connection, ignoreGuiCommands));
        init_traci();
        return;
    }
    ...
    throw cRuntimeError("TraCIScenarioNetManager received unknown self-message");
}

void TraCIScenarioNetManager::preInitializeModule(cModule* mod, const std::string& nodeId, const Coord& position, const std::string& road_id, double speed, Heading heading, VehicleSignalSet signals)
{
...
void TraCIScenarioNetManager::updateModulePosition(cModule* mod, const Coord& p, const std::string& edge, double speed, Heading heading, VehicleSignalSet signals)
{
...
void TraCIScenarioNetManager::addModule(std::string nodeId, std::string type, std::string name, std::string displayString, const Coord& position, std::string road_id, double speed, Heading heading, VehicleSignalSet signals, double length, double height, double width)
{
...
cModule* TraCIScenarioNetManager::getManagedModule(std::string nodeId)
{
    if (hosts.find(nodeId) == hosts.end()) return nullptr;
    return hosts[nodeId];
}

bool TraCIScenarioNetManager::isModuleUnequipped(std::string nodeId)
{
    if (unEquippedHosts.find(nodeId) == unEquippedHosts.end()) return false;
    return true;
}

void TraCIScenarioNetManager::deleteManagedModule(std::string nodeId)
{
...
void TraCIScenarioNetManager::executeOneTimestep()
{
...
void TraCIScenarioNetManager::subscribeToVehicleVariables(std::string vehicleId)
{
...
void TraCIScenarioNetManager::unsubscribeFromVehicleVariables(std::string vehicleId)
{
    // subscribe to some attributes of the vehicle
    simtime_t beginTime = 0;
    simtime_t endTime = SimTime::getMaxTime();
...
void TraCIScenarioNetManager::subscribeToTrafficLightVariables(std::string tlId)
{
...
void TraCIScenarioNetManager::unsubscribeFromTrafficLightVariables(std::string tlId)
{
...
void TraCIScenarioNetManager::processTrafficLightSubscription(std::string objectId, TraCIBuffer& buf)
{
...
void TraCIScenarioNetManager::processSimSubscription(std::string objectId, TraCIBuffer& buf)
{
    uint8_t variableNumber_resp;
    buf >> variableNumber_resp;
    for (uint8_t j = 0; j < variableNumber_resp; ++j) {
...
void TraCIScenarioNetManager::processVehicleSubscription(std::string objectId, TraCIBuffer& buf)
{
...
void TraCIScenarioNetManager::processSubcriptionResult(TraCIBuffer& buf)
{
...
int TraCIScenarioNetManager::getPortNumber() const
{
    int port = par("port");
    if (port != -1) {
        return port;
...
void TraCIScenarioNetManager::lifecycleEvent(SimulationLifecycleEventType eventType, cObject* details)
{
...
void TraCIScenarioNetManager::listenerRemoved()
{
    delete this;
}
